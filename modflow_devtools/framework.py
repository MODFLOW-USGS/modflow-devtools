...
import flopy
import modflow_devtools as devtools
import numpy as np
...
class Framework:
    ...
    def run(self, config, workspace):
        """
        Run the MODFLOW 6 test and compare to existing head file or
        MODFLOW-2005, MODFLOW-NWT, MODFLOW-USG, or MODFLOW-LGR run.

        Parameters
        ----------
        config : TestConfig object
            Modflow 6 test configuration object that runs the base and
            regression models, compares the results, and tears down the
            test if successful.
        workspace : Pathlike
            The test workspace
        """
        config.set_model(workspace, testModel=False)
        config.run()
        config.compare()
        if config.exfunc is not None:
            config.exfunc(config)
...
class TestConfig:
    ...
    def __init__(
        ...
        for idx, arg in enumerate(sys.argv):
            if arg[2:].lower() in list(self.targets.keys()):
                key = arg[2:].lower()
                exe0 = self.targets[key]
                exe = os.path.join(os.path.dirname(exe0), sys.argv[idx + 1])
                msg = (
                    f"replacing {key} executable "
                    + f'"{self.targets[key]}" with '
                    + f'"{exe}".'
                )
                print(msg)
                self.targets[key] = exe
        ...
        msg = self.sfmt.format("Initializing test", name)
        print(msg)
        self.name = name
        self.exfunc = exfunc
        self.simpath = None
        ...
        if htol is None:
            ...
            msg = self.sfmt.format("User specified comparison htol", htol)
            print(msg)
    ...
    def set_model(self, pth, testModel=True):
        ...
        mf6inp, mf6outp = devtools.get_mf6_files(fpth)
        self.outp = mf6outp
        ...
        # else:
        #     self.action = devtools.get_mf6_comparison(pth)
        if self.action is not None:
            if "mf6" in self.action or "mf6-regression" in self.action:
                cinp, self.coutp = devtools.get_mf6_files(fpth)

    def setup(self, src, dst):
        msg = self.sfmt.format("Setup test", self.name)
        self.originpath = src
        self.simpath = dst
        self.inpt, self.outp = devtools.setup_mf6(src=src, dst=dst)
        time.sleep(0.5)
    ...
    def setup_comparison(self, src, dst, testModel=True):
        ...
        if self.mf6_regression:
            ...
            action = devtools.setup_mf6_comparison(src, dst)
        else:
            action = devtools.get_mf6_comparison(dst)
    ...
    def run(self):
        ...
        """
        msg = self.sfmt.format("Run test", self.name)
        print(msg)
        ...
        # run mf6 models
        exe = self.targets["mf6"]
        msg = self.sfmt.format("using executable", exe)
        print(msg)
        try:
            ...
            msg = self.sfmt.format("MODFLOW 6 run", self.name)
            if success:
                print(msg)
            else:
                print(msg)
        ...
        if success:
            if self.action is not None:
                if self.action.lower() == "compare":
                    msg = self.sfmt.format("Comparison files", self.name)
                    print(msg)
                else:
                    cpth = os.path.join(self.simpath, self.action)
                    key = self.action.lower().replace(".cmp", "")
                    exe = os.path.abspath(self.targets[key])
                    msg = self.sfmt.format("comparison executable", exe)
                    print(msg)
                    if (
                        "mf6" in key
                        or "libmf6" in key
                        ...
                        npth = devtools.get_namefiles(cpth)[0]
    ...
    def compare(self):
        ...
        msgall = ""
        msg = self.sfmt.format("Comparison test", self.name)
        print(msg)
        ...
        if self.action is not None:
            ...
            # non-regression runs - for new features
                ...
                for ipos in range(len(files1)):
                    ...
                    # make comparison
                    success_tst = devtools.compare_heads(
                        None,
                        pth,
                        precision="double",
                        text=self.extdict[ext],
                        outfile=outfile,
                        files1=file1,
                        files2=file2,
                        htol=self.htol,
                    ...
                    msg = self.sfmt.format(
                        f"{self.extdict[ext]} comparison {ipos + 1}",
                        self.name,
                    )
                    print(msg)
    ...
    def _compare_heads(self, msgall, extensions="hds"):
        ...
        for idx, (fpth0, fpth1) in enumerate(zip(files0, files1)):
            ...
            msg = self.sfmt.format(
                f"{self.extdict[extension]} comparison {ipos + 1}",
